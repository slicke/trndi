...
BGReading = object
  private
    curr: BGVal; //< The current (as in at this point in time, readng)
    change: BGVal; //< The difference since last reading
    valu: BGUnit; //< The value unit in which the reading is stored
    retu: BGUnit; //< The preffered output unit, it's generally better to call a function with data
    lvl: BGValLevel; //< The API's high/low limits
    src: shortstring; //< The API that reportd this value
  ...
  public
    ...
    trend: BGTrend; //< The trend "arrow"
    date: TDateTime; //< When the reading happened
    ...
    function format(u: BGUnit; fmt: BgUnitMeta; which: BGValType = BGPrimary; rounded: boolean = false): string;
...
BGTrendHelper = type helper for BGTrend //< A type helper to transform trends to images/text
  function Img: string;
...
function BGReading.format(u: BGUnit; fmt: BgUnitMeta; which: BGValType = BGPrimary; rounded: boolean = false): string;
var
  cval: single;
  str: string;
