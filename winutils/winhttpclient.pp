...
type
  ...
  function WinHttpOpenRequest(hConnect: HINTERNET; pwszVerb, pwszObjectName,
    pwszVersion: PWideChar; pwszReferrer: PWideChar; ppwszAcceptTypes: PPWideChar;
    dwFlags: DWORD): HINTERNET; stdcall; external winhttpdll;

  function WinHttpAddRequestHeaders(hRequest: HINTERNET; pwszHeaders: PWideChar;
    dwHeadersLength: DWORD; dwModifiers: DWORD): BOOL; stdcall; external winhttpdll;
  ...
  function WinHttpSendRequest(hRequest: HINTERNET; pwszHeaders: PWideChar;
    ...
    dwTotalLength: DWORD; dwContext: DWORD_PTR): BOOL; stdcall; external winhttpdll;
...
begin
  ...
  if ProtocolPos > 0 then begin
    ...
    ProtocolPos := 1; // Börja direkt om inget protokoll anges
  ...
  begin
    ...
    Port.port := StrToIntDef(PortStr, Port.port); // Konvertera till integer, eller behåll standardport om ogiltigt
  end
  else
  begin
    ...
    try
      ...
      try
        ...
        begin
          ...
          WinHttpAddRequestHeaders(hRequest, PWideChar(Headers), Length(Headers), WINHTTP_ADDREQ_FLAG_ADD);
        end;
        ...
        try
          repeat
            ...
            if not WinHttpQueryDataAvailable(hRequest, dwSize) then
              raise Exception.Create('WinHttpQueryDataAvailable failed: ' + SysErrorMessage(GetLastError));
  ...
  try
    ...
    try
      ...
      try
        ...
        if not WinHttpSendRequest(hRequest, nil, 0, PByte(FRequestBody), Length(FRequestBody), Length(FRequestBody), 0) then
          raise Exception.Create('WinHttpSendRequest failed: ' + SysErrorMessage(GetLastError));
