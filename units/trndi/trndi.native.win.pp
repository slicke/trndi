 ...
 * - 2026-09-18: The WinHTTP transport (getURL, postURL, TestProxyURL,
 *   request, requestEx) moved verbatim to the new
 *   trndi.native.request.winhttp unit as free functions; the methods here now
 *   fetch proxy settings and delegate to that shared transport, so the
 *   LCL-free console class can use the same stack on Windows.
...
{**
  ...
  - HTTP via WinHTTP (@link(TTrndiNativeWindows.getURL)); the transport
    itself lives in @code(trndi.native.request.winhttp)
...
winutils.wintaskbar, shellapi, trndi.native.request.winhttp,
...
begin
  ...
  try
    proxy := FetchWinHttpProxy(tempInstance);
  finally
    tempInstance.Free;
  end;
...
begin
  Result := WinHttpRequest(post, baseurl, useragent, endpoint, params, jsondata,
    header, prefix, FetchWinHttpProxy(self));
end;
