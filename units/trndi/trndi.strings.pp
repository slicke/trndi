...
RS_REPORT_NODATA = 'There are no readings loaded to summarise yet.';
RS_REPORT_PERIOD = '%s to %s (%s)';
RS_REPORT_SAMPLES = '%d readings, roughly %s apart, covering %.0f%% of the period';
RS_REPORT_SPAN_HM = '%d h %d min';
RS_REPORT_SPAN_M = '%d min';
RS_REPORT_SHAPE = 'Shape of the period';
RS_REPORT_DISTRIBUTION = 'Where the readings fell';
RS_REPORT_BAND_LOW = 'Low, under %s';
RS_REPORT_BAND_UNDER = 'Under range, %s-%s';
RS_REPORT_BAND_IN = 'In range, %s-%s';
RS_REPORT_BAND_OVER = 'Over range, %s-%s';
RS_REPORT_BAND_HIGH = 'High, over %s';
RS_REPORT_NUMBERS = 'Numbers';
RS_REPORT_MEAN = 'Average';
RS_REPORT_MEDIAN = 'Median';
RS_REPORT_SD = 'Spread (standard deviation)';
RS_REPORT_CV = 'Variability (CV)';
RS_REPORT_GMI = 'Glucose Management Indicator';
RS_REPORT_GMI_VALUE = '%.1f%% (%.0f mmol/mol)';
RS_REPORT_LOWEST = 'Lowest reading';
RS_REPORT_HIGHEST = 'Highest reading';
RS_REPORT_AT = '%s at %s';
RS_REPORT_EXCURSIONS = 'Excursions';
RS_REPORT_EXCURSIONS_VALUE = '%d low, %d high (runs of %d or more readings past the limit)';
RS_REPORT_GAP = 'Longest gap';
RS_REPORT_GAP_VALUE = '%d min, from %s';
RS_REPORT_GAP_NONE = 'none beyond the usual spacing';
RS_REPORT_FOOTER = 'These are plain statistics over the readings Trndi happens to hold. They are not a medical assessment, the indicator above is not a laboratory A1c, and nothing here should be acted on without your official device and your care team.';
RS_REPORT_SAVE = 'Save...';
RS_REPORT_SAVE_TITLE = 'Save summary';
RS_REPORT_SAVE_FILTER = 'Text file|*.txt|All files|*.*';
RS_REPORT_SAVE_OK = 'Summary saved to %s';
RS_REPORT_SAVE_ERROR = 'The summary could not be saved: %s';
...
RS_HISTORY_ITEM = '» Selected Reading: %s'+sLineBreak+'Δ Difference: %s'+sLineBreak+'↔ Trend: %s'+sLineBreak+'📻 RSSI: %s'+sLineBreak+'〰️ Noise: %s'+sLineBreak+'ℹ Source: %s'+sLineBreak+'💻 Device: %s';
...
RS_ATTENTION_RESERVOIR = 'Insulin running low';
RS_ATTENTION_RESERVOIR_DESC = 'About %s units of insulin left in the pump reservoir.';
RS_ATTENTION_PUMP_BATTERY = 'Pump battery low';
RS_ATTENTION_PUMP_BATTERY_DESC = 'The pump battery is down to about %d%%.';
RS_ATTENTION_SENSOR_EXPIRY = 'Sensor expiring';
RS_ATTENTION_SENSOR_EXPIRY_DESC = 'About %d hours of sensor life left.';
RS_ATTENTION_SENSOR_EXPIRY_DESC_ONE = 'Less than an hour of sensor life left.';
