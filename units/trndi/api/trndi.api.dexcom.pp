...
begin
  ...
  begin
    ...
    LAccountID := StringReplace(
      resp,
      '"', '', [rfReplaceAll]);
    {$ifdef DEBUG} if debug_log_api then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, DEXCOM_AUTHENTICATE_ENDPOINT, resp, '']));{$endif}
    ...
    FSessionID := StringReplace(
      resp,
      '"', '', [rfReplaceAll]);
        {$ifdef DEBUG} if debug_log_api then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, DEXCOM_LOGIN_BY_ID_ENDPOINT, resp, '']));{$endif}
  end
  else begin
    ...
    {$ifdef DEBUG} if debug_log_api then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, DEXCOM_LOGIN_BY_NAME_ENDPOINT, resp, '']));{$endif}
  end;
  ...
  {$ifdef DEBUG} if debug_log_api then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, DEXCOM_TIME_ENDPOINT, LTimeResponse, '']));{$endif}
...
begin
  ...
  {$ifdef DEBUG} if debug_log_api then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, DEXCOM_VERIFY_SERIAL_NUMBER_ENDPOINT, LResponse, debugParams(LParams)]));{$endif}
