...
begin
  ...
  {$ifdef DEBUG} if DEBUG_LOG_ALERT then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, NS_STATUS, responseStr, debugParams([])]));{$endif}
...
begin
  ...
  try
    ...
    {$ifdef DEBUG} if DEBUG_LOG_ALERT then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, extras, resp, params[0]]));{$endif}
  ...
  // build a path string and run the path parser per entry, which adds up
  // across hundreds of readings.
  if not (js is TJSONArray) then
  begin
    js.Free;
    Exit;
  end;
  ...
  try
    ...
    begin
      ...
      deltaField := obj.Find('delta');
      if Assigned(deltaField) then
        deltaValue := single(deltaField.AsFloat)
      else
      if (i < arr.Count - 1) and (arr.Items[i + 1] is TJSONObject) then
      begin
          // Get the previous (older) reading's SGV
        nextObj := TJSONObject(arr.Items[i + 1]);
        prevSgv := nextObj.Get('sgv', 0);
...
begin
  ...
  try
    ...
    {$ifdef DEBUG} if DEBUG_LOG_ALERT then TrndiDLog(Format('[%s:%s] / %s'#10'%s'#10'[%s]', [{$i %file%}, {$i %Line%}, 'profile.json', responseStr, debugParams([])]));{$endif}
    if Trim(ResponseStr) = '' then
    begin
