...
type 
  TrndiExtension = class
    private 
      ...
      // The ID for the Trndi class
      API_Class_Proto : JSValue;
      // Prototype, to hold funcs etc
      JClass : JSClassDef;
      // Class definition in JS
      tab : array [0..1] of JSCFunctionListEntry;
      // Our props/funcs regged in QuickJS  (@see RegisterExtension)
    public 
      ...
      class function TrndiExtension.VerifySource(const ctx: JSContext; Buf: PChar; buf_len: integer;
                                                 filename: PChar; is_main : boolean; eval_flags:
                                                 integer = -1): JSValue;
  ...
  // Register a new module in QuickJS (= creates a "C module" with the given name and exports export
        ...
        procedure TrndiExtension.RegisterModule(var val: JSValue; var ctx : JSContext; name, export:
                                                string; module: JSModuleDef = nil);
        ...
        function init(var ctx : JSContext; m : JSModuleDef): Integer;
        cdecl;
        begin
        ...
        begin
          ...
          // Create a new "C module", using init
          JS_AddModuleExport(ctx,m,PChar(export));
          // Add export
          // Execute the script as a module
          VerifySource(ctx, std_hepler, strlen(std_hepler), '<global_helper>', False,
          JS_EVAL_TYPE_MODULE);
        ...
        procedure TrndiExtension.RegisterExtension(const ctx : JSContext; module: JSModuleDef = nil)
        ;
        cdecl;
        // JS accessible method
        function install(var ctx : JSContext; {%H-}this_val : JSValueConst; argc : Integer; argv :
                         PJSValueConstArr): JSValue;
        cdecl;
                ...
                if JS_IsFunction(ctx,OnCallBack) then
                  begin
                    ...
                    if JS_ToBool(ctx, res) then
                      TrndiDLog('TrndiExtension OnCallBack return = true')
                    else
                      TrndiDLog('TrndiExtension OnCallBack return = false');
                  end;
              finally
      ...
      function New(var ctx : JSContext; new_target : JSValueConst; argc : Integer; argv :
                   PJSValueConstArr): JSValue;
      cdecl;
      begin
        ...
        JS_DefinePropertyValueStr(ctx,Result,'args',JS_NewArray(ctx),JS_PROP_CONFIGURABLE or
        JS_PROP_WRITABLE);
      end;
      ...
      begin
        ...
        if module <> nil then
          begin
            ...
            obj := JS_NewCFunction2(ctx, PJSCFunction(@New), 'TrndiExtension', 1,
        ...
        // add "or JS_PROP_WRITABLE" to make it writable.
