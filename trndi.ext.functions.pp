...
function JSValueVal.stringify: string;
  ...
  var
    i: integer;
  begin
    result := Format('%s'#13#10, [StringOfChar('>', depth)]); // Indikera djup
...
begin
  ...
  case self.data.match of
    JD_UNSET:          Result := 'Unset';
    JD_NAN:            Result := 'NaN';
    JD_UNASSIGNED:     Result := 'Unassigned';
    JD_UNKNOWN:        Result := 'Unknown';
    JD_UNINITIALIZED:  Result := 'Uninitialized';
    JD_NULL:           Result := 'null';
...
begin
  ...
  if val.data.match = want then
    ...
    FillChar(Result, SizeOf(JSValueVal), 0); // Eller någon annan standard
end;
...
begin
  result := (a.data.match = b.data.match); // En enkel typjämförelse
end;
...
var
  arrVal: JSValueRaw;
  len, i: integer;
  pVal: PJSValueVal; // Tillfällig pekare
begin
  if val.IsObject or (JS_IsArray(ctx, val.Raw) = JS_TRUE) then begin
    ...
    begin
      ...
      for i := 0 to len - 1 do begin
        New(pVal); // Allokera minne för pVal
        pVal^ := JSParseValue(ctx, JS_GetPropertyUint32(ctx, val.Raw, i)); // Tilldela värdet
        result.data.ArrayVal.Add(pVal); // Lägg till pekaren i listan
      end;
  ...
  else if JS_IsFunction(ctx, val.raw) then begin
    result.data.match := JD_FUNC;
    result.data.Func := val.Ptr;
  end
  else if val.IsNan then
...
function JSValueValToValue(ctx: JSContext; val: JSValueVal): JSValue;
  ...
  begin
    ...
    // Loop all JSValues in the provided array
    for i := 0 to arr.Count - 1 do begin
      if (arr[i]^).data.match = JD_ARRAY then begin
...
begin
  ...
  begin
    ...
    try
      if i > 0 then
        fullMsg := fullMsg + ' ';
      fullMsg := fullMsg + string(msg); // Konvertera till Pascal-sträng utan radbrytningar
    finally
...
begin
  ...
  logFunc := JS_NewCFunction(ctx^, PJSCFunction(@JSConsoleLog), 'log', 1); // todo verofy cast
