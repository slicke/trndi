...
- **`units/trndi/api/`** — backend drivers. All subclass `TrndiAPI` (`trndi.api.pp`) and are registered in `trndi.api.registry.pp`. Contract (see `guides/API.md` and CONTRIBUTING.md): `constructor create(user, pass)` — virtual, and the only form the registry metaclass calls — sets `ua` and `baseUrl`; `connect: boolean` populates thresholds and `timeDiff`; `getReadings(...): BGResults` returns readings; errors go to `lastErr` with a `false` return. The many `trndi.api.debug_*.pp` units are synthetic backends simulating sensor conditions (missing readings, expiry, etc.) used for manual and automated testing.
