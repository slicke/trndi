...
begin
  ...
  if Pos('AccountPassword', sessionID) > 0 then begin
    result  := false;
    lastErr := sErrDexPass + ' (Dex1)';
    exit;
 end;

  if not checkSession then begin
    result  := false;
  ...
  for i := 0 to res.count - 1 do
   try
      ...
      if (calcDiff) and (i > 0) then
         result[i].update(res.items[i].FindPath('Value').AsFloat, res.items[i].FindPath('Value').AsFloat-res.items[i-1].FindPath('Value').AsFloat)
      else if calcDiff then
         result[i].update(res.items[i].FindPath('Value').AsFloat, 0)
      else
         result[i].update(res.items[i].FindPath('Value').AsFloat, BG_NO_VAL); // No val as dex doesnt provide delta

      trends := res.items[i].FindPath('Trend').AsString;

      if not TryStrToInt(trends, trendtry) then
        for trendi := low(ord(BGTrend)) to High(ord(BGTrend)) do begin  // := low(BG_TRENDS_STRING) to High(BG_TRENDS_STRING) do begin
          if BG_TRENDS_STRING[BGTrend(trendi)] = trends then begin
            result[i].trend := BGTrend(trendi);
            break;
         end;
          result[i].trend := TdPlaceholder; // High(BG_TRENDS_STRING) + 1;
       end;
